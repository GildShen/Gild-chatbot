...
st.title("Q2: Skip-gram Model - Adjust Parameters and Display Selected Sentence Lines")
...
st.sidebar.subheader("Adjust Word2Vec Parameters (Skip-gram)")
...
def train_model(sentences, vector_size, window_size):
    ...
    model = Word2Vec(
        tokenized, vector_size=vector_size, window=window_size,
...
def plot_2d(model, reduced_vectors, tokenized_sentences, selected_sentences):
    ...
    fig.update_layout(
        xaxis_title="X",
        yaxis_title="Y",
        title=f"2D Word Embeddings (Skip-gram, vector_size={vector_size}, window_size={window_size})",
        width=800,
        height=800
    )
    return fig
...
def plot_3d(model, reduced_vectors, tokenized_sentences, selected_sentences):
    ...
    fig.update_layout(
        ...
        title=f"3D Word Embeddings (Skip-gram, vector_size={vector_size}, window_size={window_size})",
        width=1000,
        height=1000
    )
    return fig

# --- Chat Input for adding new sentences ---
...
if len(st.session_state.sentences) == 0:
    ...
    if reduced_vectors is None:
        ...
        # Sidebar: choose which sentences to show lines for
        ...
        selected_sentences = st.sidebar.multiselect(
            ...
            default=available_sentences  # default is showing all
        )
...
if query_word:
    if query_word in model.wv:
        ...
        st.write(model.wv[query_word][:5])  # Show first 5 numbers for readability

        similar_words = model.wv.most_similar(query_word)
        st.write(f"**Most similar words to '{query_word}':**")
        st.table(similar_words)
    else:
        st.warning(f"⚠️ The word '{query_word}' is not in the vocabulary. Try another word.")
